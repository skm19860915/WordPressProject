...
class BookYourTravel_Accommodation_Helper extends BookYourTravel_BaseSingleton {
    ...
    function get_min_price_json() {
        ...
        if (isset($_REQUEST)) {
            ...
            if (wp_verify_nonce($nonce, 'bookyourtravel_nonce')) {
                ...
                $price = $this->get_min_future_price($accommodation_id, $room_type_id, $start_date, $end_date);
    ...
    function get_min_future_price($accommodation_id, $room_type_id = 0, $start_date = null, $end_date = null, $skip_cache = false) {
        global $wpdb, $bookyourtravel_theme_globals;
            ...
            if ($dont_calculate_totals) {
                ...
                foreach ($booking_object->submitted_extra_items_array as $submitted_extra_item) {
                    if (isset($submitted_extra_item['id']) && $submitted_extra_item['quantity']) {
                        ...
                        $booking_object->total_extra_items_price += $bookyourtravel_extra_item_helper->calculate_extra_item_total($extra_item_id, $quantity, $booking_object->adults, $booking_object->billable_children, $total_days, $booking_object->room_count);
                    }
                }
                        ...
                        for ($i = 0; $i < $price_range_count; $i++) {
                            ...
                            if (in_array($i + 1, $prices)) {
                                if ($i < (($price_range_count - 1))) {
                                    ...
                                    $groupby .= $wpdb->prepare(" OR (accommodation_static_price >= %d AND accommodation_static_price <= %d ) ", $bottom, $top);
                                } else {
                                    $groupby .= $wpdb->prepare(" OR (accommodation_price >= %d ) ", $bottom);
                                    $groupby .= $wpdb->prepare(" OR (accommodation_static_price >= %d ) ", $bottom);
                                }
                            }
                        }
