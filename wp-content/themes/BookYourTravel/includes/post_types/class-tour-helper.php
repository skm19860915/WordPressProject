...
class BookYourTravel_Tour_Helper extends BookYourTravel_BaseSingleton {
	...
	function tour_type_columns_content( $content, $column_name, $term_id )
	{
		if ( 'is_repeated' == $column_name ) {
			...
			$content = is_array($term_meta) ? intval($term_meta['tour_type_is_repeated']) : 0;
	...
	function get_min_price_json() {
		if ( isset($_REQUEST) ) {
			...
			if ( wp_verify_nonce( $nonce, 'bookyourtravel_nonce' ) ) {
				...
				$price = $this->get_min_future_price($tour_id, $start_date, $end_date);
	...
	function get_min_future_price($tour_id, $start_date = null, $end_date = null, $skip_cache = false) {
	...
	function retrieve_booking_values_from_request($dont_calculate_totals = false) {
		...
		if ( isset($_REQUEST) ) {
			...
			$booking_object->cart_price = isset($_REQUEST['cart_price']) ? intval(wp_kses($_REQUEST['cart_price'], array())) : 0;
						...
						for ( $i = 0; $i < $price_range_count; $i++ ) {
							...
							if ( in_array( $i + 1, $prices ) ) {
								if ( $i < ( ($price_range_count - 1) ) ) {
									...
									$groupby .= $wpdb->prepare(" OR (tour_static_price >= %d AND tour_static_price <= %d ) ", $bottom, $top);
								} else {
									$groupby .= $wpdb->prepare(" OR (tour_price >= %d ) ", $bottom);
									$groupby .= $wpdb->prepare(" OR (tour_static_price >= %d ) ", $bottom);
								}
			...
			<td>
				<select id="term_meta[tour_type_day_of_week]" name="term_meta[tour_type_day_of_week]">
				  <?php
					for ($i=0; $i<count($days_of_week); $i++) {
						$day_of_week = $days_of_week[$i]; ?>
				  <option <?php echo (int)$term_meta['tour_type_day_of_week'] == $i ? 'selected' : '' ?> value="<?php echo esc_attr($i); ?>"><?php echo esc_html($day_of_week); ?></option>
				  <?php } ?>
				</select>
				<p class="description"><?php esc_html_e( 'Select a start day of the week for weekly tour','bookyourtravel' ); ?></p>
			</td>
