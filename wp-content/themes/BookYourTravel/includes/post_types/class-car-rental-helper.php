...
class BookYourTravel_Car_Rental_Helper extends BookYourTravel_BaseSingleton {
	...
	function get_min_price_json() {
		if ( isset($_REQUEST) ) {
			...
			if ( wp_verify_nonce( $nonce, 'bookyourtravel_nonce' ) ) {
				...
				$price = $this->get_min_future_price($car_rental_id, $start_date, $end_date);
	...
	function get_min_future_price($car_rental_id, $start_date = null, $end_date = null, $skip_cache = false) {
	...
	function car_rentals_search_orderby( $orderby, $wp_query ) {
		...
		if ( isset($wp_query->query_vars['byt_orderby']) && $wp_query->query_vars['byt_orderby'] == 'car_rental_price' ) {
			$orderby = 'car_rental_price ' . $wp_query->query_vars['order'];
		}
						...
						for ( $i = 0; $i < $price_range_count; $i++ ) {
							...
							if ( in_array( $i + 1, $prices ) ) {
								if ( $i < ( ($price_range_count - 1) ) ) {
									...
									$groupby .= " OR (car_rental_static_price >= $bottom AND car_rental_static_price <= $top ) ";
								} else {
									$groupby .= " OR (car_rental_price >= $bottom ) ";
									$groupby .= " OR (car_rental_static_price >= $bottom ) ";
								}
							}
						}
