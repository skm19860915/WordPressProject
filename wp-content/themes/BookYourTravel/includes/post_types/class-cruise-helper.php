...
class BookYourTravel_Cruise_Helper extends BookYourTravel_BaseSingleton {
	...
	function cruise_type_columns_content( $content, $column_name, $term_id )
	{
		if ( 'is_repeated' == $column_name ) {
			...
			$content = is_array($term_meta) ? intval($term_meta['cruise_type_is_repeated']) : 0;
	...
	function get_min_price_json() {
		if ( isset($_REQUEST) ) {
			...
			if ( wp_verify_nonce( $nonce, 'bookyourtravel_nonce' ) ) {
				...
				$price = $this->get_min_future_price($cruise_id, $cabin_type_id, $start_date, $end_date);
	...
	function get_min_future_price($cruise_id, $cabin_type_id, $start_date = null, $end_date = null, $skip_cache = false) {
		...
		if ($min_price == 0 || $skip_cache) {
			...
			if ($use_referral_url && !empty($referral_url)) {
				...
				(
					...
					(
						SELECT SUM(cabin_count) number_of_available_cabins, possible_dates.the_date, price
				...
				$sql .= " GROUP BY possible_dates.the_date, availabilities.price
					...
					LEFT JOIN
					(
						SELECT SUM(DISTINCT bookings.cabin_count) number_of_booked_cabins, possible_dates.the_date booking_date
	...
	function get_min_static_from_price_by_location($location_id) {
		$min_price = 0;
	...
	function get_min_future_price_by_location($location_id, $start_date = null, $end_date = null, $skip_cache = false) {
		global $wpdb, $bookyourtravel_theme_globals;
		...
		if (!$min_price || $skip_cache) {
            ...
            if (count($cruise_ids) > 0) {
                ...
                $sql .= " GROUP BY possible_dates.the_date, availabilities.price
                    ...
                    LEFT JOIN
                    (
                        SELECT SUM(DISTINCT bookings.cabin_count) number_of_booked_cabins, possible_dates.the_date booking_date, cruise_id
                        FROM " . BOOKYOURTRAVEL_CRUISE_BOOKING_TABLE . " bookings
                        INNER JOIN (" . DISTICT_DATE_RANGE_QUERY . ") possible_dates ON possible_dates.the_date = DATE(bookings.cruise_date)
	...
	function retrieve_booking_values_from_request($dont_calculate_totals = false) {
		...
		if ( isset($_REQUEST) ) {
			...
			if ($dont_calculate_totals) {
				...
				foreach ($booking_object->submitted_extra_items_array as $submitted_extra_item) {
					if (isset($submitted_extra_item['id']) && $submitted_extra_item['quantity']) {
						...
						$booking_object->total_extra_items_price += $bookyourtravel_extra_item_helper->calculate_extra_item_total($extra_item_id, $quantity, $booking_object->adults, $booking_object->billable_children, $total_days, $booking_object->cabin_count);
					}
				}
			...
			$booking_object->cart_price = isset($_REQUEST['cart_price']) ? intval(wp_kses($_REQUEST['cart_price'], array())) : null;
			...
			if ($search_only_available || isset($wp_query->query_vars['byt_date_from']) || isset($wp_query->query_vars['byt_date_from'])) {
				...
				if ((isset($date_from) || isset($date_to))) {
					...
					(
						...
						OR
						(
							((schedule.end_date IS NOT NULL OR schedule.end_date != '0000-00-00 00:00:00') AND DATE(%s) < DATE(schedule.end_date))
					";
					...
					$fields .= $wpdb->prepare($temp_fields_sql, $date_from, $date_to, $date_from);
						...
						for ( $i = 0; $i < $price_range_count; $i++ ) {
							...
							if ( in_array( $i + 1, $prices ) ) {
								if ( $i < ( ($price_range_count - 1) ) ) {
									...
									$groupby .= $wpdb->prepare(" OR (cruise_static_price >= %d AND cruise_static_price <= %d ) ", $bottom, $top);
								} else {
									$groupby .= $wpdb->prepare(" OR (cruise_price >= %d ) ", $bottom);
									$groupby .= $wpdb->prepare(" OR (cruise_static_price >= %d ) ", $bottom);
								}
							}
						}
