jQuery(function($) {
	...
	$(".meta_fields_tabs").each(function(){
		$(this).find('ul li:first-of-type').addClass('selected');
		var tab_class = $(this).find('ul li:first-of-type').attr('class');
		var link = $(this).find('ul li:first-of-type a');
		showTabFields(link, tab_class);
	});
	
	$('.meta_fields_tabs ul li a').on('click', function(e) {
		var tab_class = $(this).parent().attr('class');
		var link = this;
		showTabFields(link, tab_class);
		
		$(document).trigger('metaTabShow');
		e.preventDefault();
	});
	
	function showTabFields(link, tab_class) {
		tab_class = tab_class.replace('selected', '');
		tab_class = tab_class.trim();
		$(link).closest('.meta_fields_tabs').find('div.tab-content').hide();
		$(link).closest('.meta_fields_tabs').find('div.' + tab_class).show();
		$(link).closest('.meta_fields_tabs').find('ul li').removeClass('selected');
		$(link).closest('.meta_fields_tabs').find('ul li.' + tab_class).addClass('selected');
	}	
    ...
    $('a.meta_box_repeatable_add').on('click', function(e) {
		...
		var rows = $('table.meta_box_repeatable').find('tbody tr');	
        var defaultImage = '';
        if ($('.meta_box_default_image').length > 0) {
            var span = $('.meta_box_default_image')[0];
            defaultImage = $(span).text();
        }        
