<?php
/**
 * The template for displaying Tour list widget
 *
...
class bookyourtravel_tour_list_widget extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        global $bookyourtravel_theme_globals, $tour_list_args, $tour_item_args;

        $tour_list_args = array();

        $tour_list_args['posts_per_page'] = isset($instance['number_of_posts']) ? (int) $instance['number_of_posts'] : 4;
        $tour_list_args['sort_by'] = isset($instance['sort_by']) ? $instance['sort_by'] : 'title';
        $sort_descending = isset($instance['sort_by']) && $instance['sort_descending'] == '1';
        $tour_list_args['sort_order'] = $sort_descending ? 'DESC' : 'ASC';
        $tour_list_args['posts_per_row'] = isset($instance['posts_per_row']) ? (int) $instance['posts_per_row'] : 4;
        $tour_list_args['show_featured_only'] = isset($instance['show_featured_only']) && $instance['show_featured_only'] == '1';
        $tour_list_args['tour_type_ids'] = isset($instance['tour_type_ids']) ? (array) $instance['tour_type_ids'] : array();
        $tour_list_args['tour_duration_ids'] = isset($instance['tour_duration_ids']) ? (array) $instance['tour_duration_ids'] : array();
        $tour_list_args['tour_tag_ids'] = isset($instance['tour_tag_ids']) ? (array) $instance['tour_tag_ids'] : array();

        $tour_list_args['display_mode'] = isset($instance['display_mode']) ? $instance['display_mode'] : 'card';
        $tour_list_args['paged'] = 1;

        $display_mode = $tour_list_args['display_mode'];

        $tour_item_args = array();
        $tour_item_args['hide_title'] = isset($instance['hide_title']) && $instance['hide_title'] == '1';
        $tour_item_args['hide_image'] = (isset($instance['hide_image']) && $instance['hide_image'] == '1');
        $tour_item_args['hide_description'] = (isset($instance['hide_description']) && $instance['hide_description'] == '1');
        $tour_item_args['hide_actions'] = (isset($instance['hide_actions']) && $instance['hide_actions'] == '1');
        $tour_item_args['hide_rating'] = isset($instance['hide_rating']) && $instance['hide_rating'] == '1';
        $tour_item_args['hide_price'] = isset($instance['hide_price']) && $instance['hide_price'] == '1';
        $tour_item_args['hide_address'] = (isset($instance['hide_address']) && $instance['hide_address'] == '1');
        ...
        do_action('bookyourtravel_widget_tour_list_before');

        get_template_part('includes/parts/tour/tour', 'list');
        do_action('bookyourtravel_widget_tour_list_after');

        /* After widget (defined by themes). */
        echo $after_widget;

        // set back to default since this is a global variable
        $tour_list_args['display_mode'] = 'card';
    ...
    function update($new_instance, $old_instance) {
        ...
        $instance['tour_duration_ids'] = $new_instance['tour_duration_ids'];
        ...
        $instance['hide_actions'] = $new_instance['hide_actions'];
        $instance['hide_rating'] = $new_instance['hide_rating'];
        $instance['hide_price'] = $new_instance['hide_price'];
        $instance['hide_address'] = $new_instance['hide_address'];
    ...
    function form($instance) {
        ...
        $tour_durations = get_categories($cat_args);        
		...
		<div class="wp-widget-styling">
			...
			<div class="w-group">
				...
				<p>
					...
					<div>
						...
						<label for="<?php echo esc_attr($this->get_field_name('tour_type_ids')); ?>_<?php echo esc_attr($type->term_id); ?>"><?php echo esc_html($type->name); ?></label>
						<br />
						<?php }?>
					</div>
				</p>
				...
				<p>
					<label><?php esc_html_e('Tour duration (leave blank to ignore)', 'bookyourtravel');?></label>
					<div>
						<?php for ($j = 0; $j < count($tour_durations); $j++) {
                        $duration = $tour_durations[$j];
                        $checked = false;
                        if (isset($instance['tour_duration_ids'])) {
                            if (in_array($duration->term_id, $instance['tour_duration_ids'])) {
                                $checked = true;
                            }

                        }
                        ?>
						<input <?php echo ($checked ? 'checked="checked"' : ''); ?> type="checkbox" id="<?php echo esc_attr($this->get_field_name('tour_duration_ids')); ?>_<?php echo esc_attr($duration->term_id); ?>" name="<?php echo esc_attr($this->get_field_name('tour_duration_ids')); ?>[]" value="<?php echo esc_attr($duration->term_id); ?>">
						<label for="<?php echo esc_attr($this->get_field_name('tour_duration_ids')); ?>_<?php echo esc_attr($duration->term_id); ?>"><?php echo esc_html($duration->name); ?></label>
						<br />
						<?php }?>
					</div>
				</p>                
            ...
            ?>
						...
						<label for="<?php echo esc_attr($this->get_field_name('tour_tag_ids')); ?>_<?php echo esc_attr($tag->term_id); ?>"><?php echo esc_html($tag->name); ?></label>
						<br />
						<?php }?>
					</div>
				</p>
			...
			<div class="w-group">
				...
				<p>
					...
					<label for="<?php echo esc_attr($this->get_field_id('hide_address')); ?>"><?php esc_html_e('Hide item address?', 'bookyourtravel');?></label>
				</p>
...
add_shortcode('byt_widget_tour_list', 'byt_widget_tour_list');
function byt_widget_tour_list($atts) {

    if (isset($atts['tour_type_ids']) && $atts['tour_type_ids'] == '') {
        unset($atts['tour_type_ids']);
    }

    if (isset($atts['tour_duration_ids']) && $atts['tour_duration_ids'] == '') {
        unset($atts['tour_duration_ids']);
    }    

    if (isset($atts['tour_tag_ids']) && $atts['tour_tag_ids'] == '') {
        unset($atts['tour_tag_ids']);
    }
    ...
    if (isset($atts["tour_type_ids"]) && !is_array($atts["tour_type_ids"]) && !empty($atts["tour_type_ids"])) {
      $atts["tour_type_ids"] = explode(',', $atts["tour_type_ids"]);
    }

    if (isset($atts["tour_duration_ids"]) && !is_array($atts["tour_duration_ids"]) && !empty($atts["tour_duration_ids"])) {
        $atts["tour_duration_ids"] = explode(',', $atts["tour_duration_ids"]);
      }    

    if (isset($atts["tour_tag_ids"]) && !is_array($atts["tour_tag_ids"]) && !empty($atts["tour_tag_ids"])) {
      $atts["tour_tag_ids"] = explode(',', $atts["tour_tag_ids"]);
    }
    ...
    ob_start();
    the_widget('bookyourtravel_tour_list_widget', $atts, $widget_args);
    $output = ob_get_clean();
