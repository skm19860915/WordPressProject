<?php
/**
 * The template for displaying Accommodation list widget
 *
...
class bookyourtravel_accommodation_list_widget extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        global $bookyourtravel_theme_globals, $accommodation_list_args, $accommodation_item_args;

        $accommodation_list_args = array();

        $accommodation_list_args['posts_per_page'] = isset($instance['number_of_posts']) ? (int) $instance['number_of_posts'] : 4;
        $accommodation_list_args['sort_by'] = isset($instance['sort_by']) ? $instance['sort_by'] : 'title';
        $sort_descending = isset($instance['sort_descending']) && $instance['sort_descending'] == '1';
        $accommodation_list_args['sort_order'] = $sort_descending ? 'DESC' : 'ASC';
        $accommodation_list_args['posts_per_row'] = isset($instance['posts_per_row']) ? (int) $instance['posts_per_row'] : 4;
        $accommodation_list_args['show_featured_only'] = isset($instance['show_featured_only']) && $instance['show_featured_only'] == '1';
        $accommodation_list_args['accommodation_type_ids'] = isset($instance['accommodation_type_ids']) ? (array) $instance['accommodation_type_ids'] : array();
        $accommodation_list_args['accommodation_tag_ids'] = isset($instance['accommodation_tag_ids']) ? (array) $instance['accommodation_tag_ids'] : array();
        $accommodation_list_args['display_mode'] = isset($instance['display_mode']) ? $instance['display_mode'] : 'card';
        $accommodation_list_args['paged'] = 1;

        $display_mode = $accommodation_list_args['display_mode'];

        $accommodation_item_args = array();

        $accommodation_item_args['hide_title'] = isset($instance['hide_title']) && $instance['hide_title'] == '1';
        $accommodation_item_args['hide_image'] = (isset($instance['hide_image']) && $instance['hide_image'] == '1');
        $accommodation_item_args['hide_description'] = (isset($instance['hide_description']) && $instance['hide_description'] == '1');
        $accommodation_item_args['hide_actions'] = (isset($instance['hide_actions']) && $instance['hide_actions'] == '1');
        $accommodation_item_args['hide_rating'] = isset($instance['hide_rating']) && $instance['hide_rating'] == '1';
        $accommodation_item_args['hide_price'] = isset($instance['hide_price']) && $instance['hide_price'] == '1';
        $accommodation_item_args['hide_stars'] = isset($instance['hide_stars']) && $instance['hide_stars'] == '1';
        $accommodation_item_args['hide_address'] = (isset($instance['hide_address']) && $instance['hide_address'] == '1');
        ...
        do_action('bookyourtravel_widget_accommodation_list_before');
        get_template_part('includes/parts/accommodation/accommodation', 'list');
        do_action('bookyourtravel_widget_accommodation_list_after');

        /* After widget (defined by themes). */
        echo $after_widget;

        // set back to default since this is a global variable
        $accommodation_list_args['display_mode'] = 'card';
    ...
    function update($new_instance, $old_instance) {
        ...
        $instance['hide_stars'] = $new_instance['hide_stars'];
    ...
    function form($instance) {
		...
		<div class="wp-widget-styling">
            ...
            ?>
						...
						<label for="<?php echo esc_attr($this->get_field_name('accommodation_type_ids')); ?>_<?php echo esc_attr($type->term_id); ?>"><?php echo esc_html($type->name); ?></label>
						<br />
						<?php }?>
					</div>
				</p>
            ...
            ?>
						...
						<label for="<?php echo esc_attr($this->get_field_name('accommodation_tag_ids')); ?>_<?php echo esc_attr($tag->term_id); ?>"><?php echo esc_html($tag->name); ?></label>
						<br />
						<?php }?>
					</div>
				</p>
			...
			<div class="w-group">
				...
				<p>
					<input type="checkbox"  <?php echo ($instance['hide_stars'] == '1' ? 'checked="checked"' : ''); ?> class="checkbox" id="<?php echo esc_attr($this->get_field_id('hide_stars')); ?>" name="<?php echo esc_attr($this->get_field_name('hide_stars')); ?>" value="1" />
					<label for="<?php echo esc_attr($this->get_field_id('hide_stars')); ?>"><?php esc_html_e('Hide item stars?', 'bookyourtravel');?></label>
				</p>
				<p>
...
add_shortcode('byt_widget_accommodation_list', 'byt_widget_accommodation_list');
function byt_widget_accommodation_list($atts) {

    if (isset($atts['accommodation_type_ids']) && $atts['accommodation_type_ids'] == '') {
        unset($atts['accommodation_type_ids']);
    }

    if (isset($atts['accommodation_tag_ids']) && $atts['accommodation_tag_ids'] == '') {
        unset($atts['accommodation_tag_ids']);
    }
    ...
    extract(shortcode_atts(
        array(
            ...
            'accommodation_type_ids' => array(),
            'show_featured_only' => '0',
            'show_fields' => 'title,image,actions,description,price,rating,stars,address',
            'css' => '',
        ),
        $atts
    ));

    if (isset($atts["accommodation_tag_ids"]) && !is_array($atts["accommodation_tag_ids"]) && !empty($atts["accommodation_tag_ids"])) {
        $atts["accommodation_tag_ids"] = explode(',', $atts["accommodation_tag_ids"]);
    }

    if (isset($atts["accommodation_type_ids"]) && !is_array($atts["accommodation_type_ids"]) && !empty($atts["accommodation_type_ids"])) {
        $atts["accommodation_type_ids"] = explode(',', $atts["accommodation_type_ids"]);
    }
    ...
    $atts['hide_stars'] = !in_array('stars', $show_fields);
    ...
    ob_start();
    the_widget('bookyourtravel_accommodation_list_widget', $atts, $widget_args);
    $output = ob_get_clean();
