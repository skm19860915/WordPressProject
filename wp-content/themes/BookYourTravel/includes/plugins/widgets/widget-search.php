...
class bookyourtravel_search_widget extends WP_Widget {
			...
			foreach($filters[$block_id] as $fi => $filter) {
				...
				switch ($type) {
					...
					case 'location-by-type':
						...
						$this->render_filter_location_by_type_select($widget_id, $label, 'l[]', $type_ids, $request_values, $internal_index);
						break;
	...
	function render_filter_location_by_type_select($widget_id, $filter_label, $filter_name, $filter_type_ids, $values_array, $filter_index) {
		global $bookyourtravel_location_helper;

		$file_name_short = str_replace("[]", "", $filter_name);

		$hidden_val = '';
		if (isset($_GET['hlt' . $filter_index . '' . $file_name_short])) {
			$hidden_val = wp_kses($_GET['hlt' . $filter_index . '' . $file_name_short], array());
		}

		echo '<div class="select">';
		echo sprintf('<label for="lbt_%s">%s</label>', $widget_id . '_' . $filter_index . '_' . $filter_name, $filter_label);
		echo sprintf('<input type="hidden" id="hlt_%s" name="hlt%s" value="%s">', $filter_index . '_' . $file_name_short, $filter_index . '' . $file_name_short, $hidden_val);
		echo sprintf('<select class="filter-locations-by-type" data-relid="hlt_%s" id="lbt_%s" name="%s">', $filter_index . '_' . $file_name_short, $widget_id . '_' . $filter_index . '_' . $filter_name, $filter_name);
		echo '<option value=""></option>';
		...
		<div class="inner">
			...
			<dl class="filters <?php echo esc_attr($filters_addon_class); ?>">
				...
				<dd data-<?php echo esc_attr($filter_entity_type); ?>-filter="<?php echo sprintf("%d", $i); ?>" class="filter-type">
					...
					<?php foreach ($filter_types as $filter_key => $filter_value) {
