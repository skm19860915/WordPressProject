		...
		if ($this->content_type == 'accommodation' ||
			...
			if ( isset($_POST['entry_id']) ) {
				...
				if ($this->entry) {
					...
					$this->entry->post->post_status = $this->publish_immediately() ? 'publish' : 'draft';
			...
			if ( $this->entry_id == 0 ) {
				...
				$post_array = array(
					...
					'post_status'   => $this->publish_immediately() ? 'publish' : 'draft',
				);
            ...
            if ($this->entry_id > 0) {
                if (isset($_POST['featured-image-id'])) {
                    ...
                    if (count($featured_image_id_array) > 0) {
