...
class BookYourTravel_Tour_Schedule_Admin extends BookYourTravel_BaseSingleton {
	...
	function enqueue_admin_scripts_styles() {
		...
		if (isset($_POST['tour_id'])) {
			$tour_id = intval(wp_kses($_POST['tour_id'], array()));
		...
		wp_localize_script( 'bookyourtravel-admin-script', 'BYTAdminTours', array(
			...
			'tourTypeIsRepeated' => $tour_type_is_repeated,
			'tourStartDateValue' => $start_date,
			'tourEndDateValue' => $end_date,
	 	) );
	}
	...
	function tour_schedule_add_screen_options() {
		...
		$option = 'per_page';
		$args = array('label' => esc_html__('Schedule', 'bookyourtravel'),'default' => 50,'option' => 'tour_schedule_per_page');
	...
	*/
	...
	function handle_form_submit() {
		...
		if (isset($_POST['insert']) && check_admin_referer('bookyourtravel_nonce')) {
			...
			} else if(empty($price_regular) || $price_regular === 0) {
			...
			if (!empty($error)) {
				...
				$bookyourtravel_tour_helper->create_tour_schedule($season_name, $tour_id, $start_date, $price_regular, $price_child, $max_people, $end_date);
			...
			if (!empty($error)) {
				...
				$bookyourtravel_tour_helper->update_tour_schedule($schedule_id, $season_name, $start_date, $tour_id, $price_regular, $price_child, $max_people, $end_date);
	...
	function render_entry_form() {
		...
		$tours_select = '<select id="tour_id" name="tour_id" class="tours_select">';
		...
		echo '	<th scope="row" valign="top">' . esc_html__('Tour type', 'bookyourtravel') . '</th>';
		echo '  <td>';
		$tour_type_repeated_style = '';
		$tour_type_not_repeated_style = '';
		if ($tour_type_is_repeated) {
			$tour_type_repeated_style = '';
			$tour_type_not_repeated_style = 'display:none';
		} else {
			$tour_type_repeated_style = 'display:none';
			$tour_type_not_repeated_style = '';
		}
		echo '		<span class="is_repeated" style="' . $tour_type_repeated_style . '">' . esc_html__('Is repeated', 'bookyourtravel') . '</span>';
		echo '		<span class="is_not_repeated" style="' . $tour_type_not_repeated_style . '">' . esc_html__('Is not repeated', 'bookyourtravel') . '</span>';
		echo '<p><small>' . __('Please note: Non repeated tour types do not have an end date.', 'bookyourtravel') . '</small></p>';
		echo '	</td>';
		echo '</tr>';
		...
		echo '	<td>';
		echo '  <select id="max_people" name="max_people">';
		for ($i=1;$i<101;$i++) {
			echo '  <option value="' . $i . '" ' . ($max_people == $i ? "selected" : "") . '>' . $i . '</option>';
		}
		echo '  </select>';
		echo '	</td>';
		echo '</tr>';
		...
		echo '	<td><input type="text" name="price_regular" id="price_regular" value="' . esc_attr($price_regular) . '" /></td>';
		echo '</tr>';
